...
## 7.1. 구조적 프로시저
✅ Verilog에서 구조적 프로시저(Structured Procedures)
...
2. 할당 연산자
    ...
    - 즉시 할당: `=`는 즉시 값을 할당하며, 주로 `initial` 블록에서 사용됩니다.

    - 비동기 할당: `<=`는 이벤트 큐에 등록되어 이벤트 발생 시 값을 할당하며, 주로 `always` 블록에서 사용됩니다.
...
### 7.2.1. 블로킹 문장
✅ Verilog에서 블로킹 문장(Blocking Statement)
...
### 7.2.2. 논블로킹 문장
✅ Verilog에서 논블로킹 문장(Non-Blocking Statement)
...
### 7.3.2. 사건 기반 타이밍 제어
✅ Verilog에서 사건 기반 타이밍 제어(Event-Based Timing Control)
Verilog의 **사건 기반 타이밍 제어**는 신호의 변화나 특정 사건에 의해 트리거되어 절차적 블록을 실행하는 방법입니다. 

이는 정규 사건 제어, 명명된 사건 제어, 사건 OR 제어로 나뉩니다.

#### 📌 1. 정규 사건 제어 (Regular Event Control)

- 기호: `@`

- 사용법: @(posedge clk) (클록 상승 엣엣지에서 실행)
...
#### 📌 2. 명명된 사건 제어 (Named Event Control)
...
Verilog의 **준위 구동 타이밍 제어**는 신호의 레벨에 따라 절차적 블록을 실행하는 방법입니다. 
...
### 7.7.1 블록의 형태
✅ Verilog에서 블록의 형태: 순차 처리 블록과 병렬 처리 블록
...
명명된 블록은 이름을 지정하여 특정 블록을 참조하거나 종료할 수 있는 기능입니다. 이는 절차적 작업을 더 효율적으로 관리하는 데 유용합니다.
...
1. 기본 명명된 블록
    ...
    disable block_name; // 특정 조건에서 명명된 블록 종료 가능
    ```

    ✔ 설명:

    - block_name이라는 이름이 붙은 명명된 블록을 정의하고, 필요 시 종료합니다.
...
##### 주의사항
- 명명된 블록은 절차적 블록 내에서만 참조할 수 있다.

- 명명된 블록은 설계 계층 구조의 일부분이다.
    - 명명된 블록의 변수는 계층적 이름 참조를 사용해서 접근할 수 있다.

- 명명된 블록을 종료하기 위해서는 명확한 종료 조건이 필요하다.
...
#### 및 명명된 블록의 무효화
✅ Verilog에서 명명된 블록의 무효화 (Disable Named Blocks)

Verilog에서 명명된 블록은 begin 또는 fork로 시작하는 블록에 이름을 지정하여 참조하거나 종료할 수 있는 기능입니다.

**무효화(Disable)**는 disable <block_name> 구문을 사용하여 해당 블록의 실행을 중단합니다.

1. 기본 개념
    - 명명된 블록: `begin` 또는 `fork`로 시작하는 블록에 이름을 지정하여 참조할 수 있습니다.
...
```
4. 동작 방식
- `disable` 키워드는 현재 실행 중인 명명된 블록만 종료합니다. 
...
### 7.8.2. 조건 생성
✅ Verilog에서 조건 생성 (Conditional Generate)
...
### 7.8.3. 케이스 생성
✅ Verilog에서 케이스 생성 (Case Generate)
